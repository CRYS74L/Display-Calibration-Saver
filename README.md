...
1. 从 [Releases](https://github.com/CRYS74L/Display-Calibration-Saver/releases/latest) 下载并解压最新发布包。
2. 双击 `Run.cmd`。
...
- 支持自定义输出文件名、保存位置和倒计时时间
- 严格检查 ICC 声明长度、标签表、重复 `vcgt`、偏移、对齐和文件边界
- 通过同目录临时文件进行原子写入，失败时不会破坏已有输出文件
...
本工具使用 Windows GDI `GetDeviceGammaRamp` 接口。该接口依赖显卡驱动与显示路径，并不支持所有硬件、HDR 或高级色彩场景；其他程序或系统事件也可能随时覆盖当前曲线。
...
- 当前版本要求 `vcgt` 是配置文件中的最后一个数据块，才能在不移动其他标签的情况下安全扩展。
- ICC 头部声明的配置文件长度必须与实际文件长度一致，并且只能存在一个 `vcgt` 标签条目。
...
如果捕获结果是完全线性的 Identity Gamma Ramp，工具会在写入前警告并要求确认；这通常表示预览没有生效或选错了显示器。
